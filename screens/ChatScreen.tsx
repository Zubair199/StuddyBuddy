    ...
    api.createNewMessage(requestData).then((resp) => {
      if (resp) {
        if(resp.data.blockedby===true){
          console.log("hererer")
          setBlocker(true)
          Alert.alert("Blocked by "+capitalizes(userother1),"Sorry cannot send messages! you have been blocked by "+capitalizes(userother1),[{text: "Ok",style: "cancel"}],{ cancelable: false })
        }
        else{
          setMessages((previousMessages: any) =>
